...
dropData$dropout2 <- if_else(dropData$dropout == TRUE, 
...
#next graphing optimizers
optData <- combinedTidy %>% filter( hidden_layers == 2, loss == "NLL_withDropout")

ggplot( data = subset(optData, !is.na(value)), 
        aes( x = trainTask, y = value, color = taskNum, 
             group = taskNum) ) +
  geom_point() +
  geom_line() +
  ...
  theme_bw() +
  guides(color = guide_legend(ncol = 2)) +
  labs( title = "Effect of Optimizer on Forgetting",
        x = "Task Trained On",
        y = "Accuracy",
        color = "Task Tested On")
